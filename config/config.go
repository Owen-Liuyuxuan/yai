...
func NewConfig() (*Config, error) {
    ...
    return &Config{
        ai: AiConfig{
            ...
            baseURL:     viper.GetString(openai_base_url), // Add this
        },
        user: UserConfig{
...
func WriteConfig(key string, write bool) (*Config, error) {
    ...
    viper.Set(openai_model, "deepseek-chat") // Change default model to DeepSeek
    viper.SetDefault(openai_proxy, "")
    viper.SetDefault(openai_temperature, 0.2)
    viper.SetDefault(openai_max_tokens, 1000)
    viper.SetDefault(openai_base_url, "https://api.deepseek.com") // Set DeepSeek URL
